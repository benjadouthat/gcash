...
def extraer_tarjeta_pdf(archivo, clave: str = "") -> pd.DataFrame:
    ...
    Asigna Fuente específica (TNX, TNX Visa, TGM) y separa comprobante.
    """
    ...
    try:
        ...
        if getattr(reader, "is_encrypted", False):
            try:
                ...
                st.error(f"⚠️ No se pudo desencriptar '{filename}'. "
    ...
    # Texto de muestra para detectar la tarjeta
    sample_text = ""
    ...
    for page in reader.pages:
        ...
        # Strategy B: sequential date → description → amount
        i = 0
        while i < len(lines):
            if re.match(r"^\d{2}/\d{2}/\d{2,4}$", lines[i]):
                ...
                if re.search(r"[\d]", monto_cand) and not re.match(r"^\d{2}/\d{2}", monto_cand):
                    ...
                    if monto is not None and desc:
                        # El comprobante a veces viene en la línea extra
                        combined = f"{desc} {extra}".strip()
                        comp, clean_desc = _split_comprobante_from_desc(combined)
                        # Si no se separó bien, intentar solo sobre desc
                        if not comp:
                            comp, clean_desc = _split_comprobante_from_desc(desc)
                        data.append({
                            "Fecha": normalize_date(fecha_s),
