...
def leer_excel_galicia(archivo) -> pd.DataFrame:
    """Parse Banco Galicia extract (ARS or USD). Monto = Crédito − Débito."""
    ...
    # FIX: expenses (Débito) are negative
    df["Monto"] = df["Crédito"].fillna(0.0) - df["Débito"].fillna(0.0)
    ...
    return df[["Fecha", "Movimiento", "Monto", "Moneda", "Categoria", "Fuente"]].copy()
...
def metric_row(df: pd.DataFrame, moneda: str):
    ...
    c4.metric("Neto", f"{neto:,.2f}", delta_color="normal")
...
def main():
    ...
    # Process
    if st.button("🔄 Procesar archivos", type="primary") or st.session_state.get("df_final") is not None:
        if "df_final" not in st.session_state or st.button:
            # Re-process only when button pressed or no previous data
            pass

    process = st.button("🔄 Procesar / Actualizar", type="primary", use_container_width=True)

    if process or ("df_final" in st.session_state and st.session_state.df_final is not None and not process):
        if process:
        ...
        if only_dups:
            mask &= df_final["Posible_Duplicado"] == True  # noqa: E712
        ...
        with tab_mov:
            ...
            st.dataframe(
                df_filt.style.format({"Monto": "{:,.2f}"}, na_rep="—"),
        ...
        with tab_res:
            if df_filt.empty:
                ...
                for moneda in moneda_sel:
                    ...
                    st.dataframe(resumen.style.format({"Monto_abs": "{:,.2f}"}), use_container_width=True)
        ...
        with tab_rep:
            ...
            if report.empty:
                ...
                st.dataframe(
                    report.style.format({"Ingreso": "{:,.2f}", "Gasto": "{:,.2f}", "Neto": "{:,.2f}"}),
                    use_container_width=True,
                )
                for moneda in moneda_sel:
        ...
        with tab_dup:
            dups = df_final[df_final["Posible_Duplicado"]]
            st.write(
